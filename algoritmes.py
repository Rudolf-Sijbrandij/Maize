...
def unvisited_neighbour(rows, columns, cur, vis, maze_type, dif, maze):
    ...
    anders wel verder kan. Als hij wel verder kan, dan returned hij de coordinates van de nieuwe cell.
    ...
    Args:
        ...
        cur (list):     De coordinates van de huidige cell waarvoor je naar neighbours kijkt.
        vis (list):     Een lijst van coordinates van al eerder bezochte cellen.
        maze_type (int):De maze type, als het 0 is is het een normale maze, als het 1 is is het een disjointed maze.
                            Alleen relevant als het om het genereren van een maze gaat
        dif (int):      Een getal, 0 of 1, dat aangeeft of de functie wordt gebruikt door de dfs() functie of door
                            de solve() functie, gaat erom of er stappen van 1 of van 2 gemaakt worden.
        maze (list):    Alleen relevant voor solve functie, laat de functie kijken of de stap die hij maakt wel op het
                            pad is en niet door een muur heen gaat

    Returns:
        list:   Een lijst met coordinates van een neighbour 1 stap verder en 2 stappen verder
...
def dfs(rows, columns, maze_type):
    """
    Door middel van de dimensies van de gebruiker maakt het algoritme een doolhof door middel van het
...
def solver(maze):
    """
    Zoekt een pad naar het einde van de maze vanaf het begin door middel van depth first search maze solving algorithm.
    ...
    Returns:
        list: een lijst van coordinates van het pad naar het einde van de maze.
    """
    current = [0, 0]
    solution = [current]
    visited = []
